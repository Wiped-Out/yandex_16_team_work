...
def gen_film_like(user_id: UUID4) -> tuple:
    film_id = SystemRandom().choice(films)
...
def gen_film_progress(user_id: UUID4) -> tuple:
    ...
    timestamp = SystemRandom().randint(0, 6000)
...
def produce_msgs(producer: KafkaProducer) -> None:
    ...
    while i < settings.MAX_NUMBER_USERS:
        ...
        # Likes
        for _ in range(SystemRandom().randint(0, 20)):
        ...
        # Progress
        for _ in range(SystemRandom().randint(0, 30)):
        ...
        # Comments
        for _ in range(SystemRandom().randint(0, 10)):
        ...
        # Sleeping time
        sleep_time = SystemRandom().randint(
...
def main():
    producer = KafkaProducer(
        bootstrap_servers=[f'{settings.KAFKA_HOSTNAME}:{settings.KAFKA_PORT}'])
